...
(async() => {
    ...
    if (resetDayLeft) {
        infoList.push(`Đặt lại dung lượng: còn lại ${resetDayLeft} ngày`);
    }
    if (expire) {
...
function getUserInfo(url) {
    ...
    return new Promise((resolve, reject) =>
        $httpClient.head(request, (err, resp) => {
            ...
            reject("Liên kết đăng ký của bạn không phản hồi dung lượng");
        })
    );
}
...
function sendNotification(usageRate, expire, infoList) {
    ...
    if (usageRate > count.usageRate && resetDay != today) {
        $notification.post(
            `${title} | Dung lượng truy cập còn lại ít hơn ${Math.ceil(100 - usageRate)}%`,
            subtitle,
            body
        );
    ...
    if (resetDayLeft && resetDayLeft < count.resetDayLeft && resetDay != today) {
        $notification.post(
            `${title} | Dung lượng sẽ được đặt lại sau ${resetDayLeft} ngày`,
            subtitle,
            body
        );
    ...
    if (resetDay == today && count.resetDay && usageRate < 5) {
        $notification.post(`${title} | Dung lượng truy cập đã được đặt lại`, subtitle, body);
        count.resetDay = 0;
    }
    if (expire) {
        ...
        if (diff < count.expire) {
            $notification.post(
                `${title} | Thời gian còn lại của gói ít hơn ${Math.ceil(diff)} ngày`,
                subtitle,
                body
            );
            count.expire -= 5;
